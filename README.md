...
| 限速器形状 | 重传 ≥1%，但延迟很平 | 不自动限速；先用可控 iperf3 对端触发式复测 |
...
### 无线丢包与 policer 不能只看重传率

两者都可能出现 2%–3% 重传，但处置方向不同：

- 无线调度/换手通常同时拉开 RTT 尾部；降一点服务端速率未必减少射频层丢包。
- policer 常表现为重传突然升高、延迟形状仍较平；只有在可控测速中找到拐点，才适合推导速率。

项目把真实 4G 样本（RTT 208/344 ms、minRTT 164 ms、重传约 2.9%）和稳定延迟的 policer 样本锁进了回归测试，防止再次把二者合并。
...
`v0.1.0` 是观测与建议阶段。下一阶段以 DMIT 真机数据校准阈值、画像稳定性和路由实验收益；在此之前不会加入自动 apply。
