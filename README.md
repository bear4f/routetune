...
覆盖 RTT 取所有前缀里**最宽的那个典型轮次**（不是有史以来最差的一轮——最大值只增不减），
再与 150 ms 下限取较大者。这条下限的论证来自 [tcpfit](https://github.com/Kylin010/tcpfit)：
代价是不对称的，估高只多付一点 BBR 超发，估低是每个远端客户端都撞得到、却查不出原因的
硬顶。tcpfit 因为看不到客户端只能硬编码 150 ms，routetune 能看到，所以取实测与下限的较大者。
...
`v0.3.0` 起全局层可以落地（`tune --yes`，默认预演、写前快照、可 `revert`）。per-route 层仍然只出建议不自动执行——写错路由 metric 是事故，打印错了只浪费一分钟。
